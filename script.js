...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const prevButton = document.querySelector('.card-img-prev');
    const nextButton = document.querySelector('.card-img-next');
    const slideWidth = slides[0].getBoundingClientRect().width + 20; // Adicione a margem de 10px em cada lado
    const totalSlides = slides.length;
    let currentIndex = 0;
    ...
    prevButton.addEventListener('click', function() {
        currentIndex = (currentIndex === 0) ? totalSlides - 1 : currentIndex - 3;
        updateSlides();
    });

    nextButton.addEventListener('click', function() {
        currentIndex = (currentIndex >= totalSlides - 1) ? 0 : currentIndex + 3;
        updateSlides();
    });
